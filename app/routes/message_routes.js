...
// Get message api functions for re-use
...
router.post('/messages', requireToken, (req, res, next) => {
  ...
  console.log('message creation!', req.body.message)
...
router.patch('/messages/:id', requireToken, removeBlanks, (req, res, next) => {
  ...
  Message.findById(req.params.id)
    ...
    // if that succeeded
    .then(index)
    // Find all the messages again and send back full list
    .then(messages => res.status(204).json({ messages: messages }))
    // if an error occurs, pass it to the handler
    .catch(next)
})
